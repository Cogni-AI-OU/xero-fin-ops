...
Commands:
    ...
    post    Post a draft manual journal.
...
Examples:
    ...
    # Post a draft journal (Dry Run)
    ./xero_journal_manager.py post --journal-id <ID> --dry-run

    # Post a draft journal (Apply)
    ./xero_journal_manager.py post --journal-id <ID>
...
def post_journal(api_client, tenant_id, journal_id, dry_run=False):
    ...
    try:
        ...
        if dry_run:
            print("[DRY RUN] Would post journal (change status from DRAFT to POSTED)", file=sys.stderr)
...
def main():
    ...
    post_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simulate the post without applying changes",
    )
    ...
    if args.command == "view":
        ...
        post_journal(api_client, tenant_id, args.journal_id, args.dry_run)
    else:
        parser.print_help()
